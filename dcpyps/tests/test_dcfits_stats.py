...
def test_regression_stats():
    from dcpyps.dcfits.stats import lik_intervals, SSR
    ...
    errs = ObservedInformation(result.rd['x'], ssr.SSR, X.size)
...
def test_OservedInformation():
    ...
    from dcpyps.dcfits.stats import ObservedInformation
    errs = ObservedInformation(result.rd['x'], ssr.SSR, dataset.X.size)
    
    assert_almost_equal(errs.approximateSD[0], 5.2168715720984222, delta = 1e-9)
    assert_almost_equal(errs.approximateSD[1], 1.5729459621937478, delta = 1e-9)
    
    from dcpyps.dcfits.stats import EstimateErrors
    ll = EstimateErrors(equation, dataset, errs.approximateSD)
    
    assert_almost_equal(ll.Llimits[0][1], 16.001557908087079, delta = 1e-6)
    assert_almost_equal(ll.Llimits[1][0], 1.4776049294397993, delta = 1e-6)
    assert_almost_equal(ll.Llimits[1][1], 8.9233105381283568, delta = 1e-6)
